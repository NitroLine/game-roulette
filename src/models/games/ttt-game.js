...
export class TTTGame extends BaseGame {
    ...
    getStatus() {
        ...
        return {
            ...
            side: this._getPlayerSide(winner)
        };
    }
    ...
    _getWinnerSide() {
        for (let i = 0; i < this.dimention; ++i) {
            ...
            for (let j = 0; j < this.dimention; ++j) {
                if (this.field[i][j] === Field.CROSS) {
                    crossHorCount++;
                }

                if (this.field[i][j] === Field.ZERO) {
                    zerosHorCount++;
                }

                if (this.field[j][i] === Field.CROSS) {
                    crossVerCount++;
                }

                if (this.field[j][i] === Field.ZERO) {
                    zerosVerCount++;
                }

                if (crossHorCount === this.dimention || crossVerCount === this.dimention) {
                    return Field.CROSS;
                }

                if (zerosHorCount === this.dimention || zerosVerCount === this.dimention) {
                    return Field.ZERO;
                }
            }
        }
        ...
        for (let i = 0; i < this.dimention; ++i) {
            if (this.field[i][i] === Field.CROSS) {
                crossMajorCount++;
            }

            if (this.field[i][i] === Field.ZERO) {
                zerosMajorCount++;
            }

            if (this.field[i][this.dimention - i - 1] === Field.CROSS) {
                crossMinorCount++;
            }

            if (this.field[i][this.dimention - i - 1] === Field.ZERO) {
                zerosMinorCount++;
            }
        }

        if (crossMajorCount === this.dimention || crossMinorCount === this.dimention) {
            return Field.CROSS;
        }

        if (zerosMajorCount === this.dimention || zerosMinorCount === this.dimention) {
            return Field.ZERO;
        }
